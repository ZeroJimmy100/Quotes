...
class QuoteManager(models.Manager):
    def getValid(self, post_data):
        errors={}
        if len(post_data['get_Author']) < 3:
            errors["author"] = "Invalid Author"
        if len(post_data['quote_add']) < 10:
            errors["quote"] = "Quote should be at least 10 characters or more"
        return errors
...
class Quote(models.Model):
    author = models.ForeignKey(Author, related_name="quotes_written", on_delete=models.CASCADE)
