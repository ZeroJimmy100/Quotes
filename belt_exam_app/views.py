...
from .models import User, UserManager, Quote, Author, QuoteManager, Likes, CheckManager
...
def AddQuote(request):
    ...
    if len(quoteError) > 0:
        ...
        create_author = Author.objects.create(name = request.POST['get_Author'])
    # this_user_id = User.objects.get(id = request.session['user'])
        create_quote = Quote.objects.create(author = create_author, quote = request.POST['quote_add'], uploaded_by = this_user_id)
    # ok_user = this_user_id.id
        return redirect(f"/success/{ok_user}")
...
def view_other_users(request, this_user):
    ...
    print("this is my user", this_user)
...
def putLikes(request):
    ...
    # users_who_like = getQuote.users_who_like
    # if this_user not in users_who_like:
    #     users_who_like
        
    #length_of_likes = len(users_who_like)

    return redirect(f"/success/{my_id}")
